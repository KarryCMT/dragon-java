...
public class DgCommentServiceImpl extends ServiceImpl<DgCommentMapper, DgComment> implements DgCommentService {
    ...
    public Page page(DgCommentQueryReq req) {
        ...
        Map<String, RbMember> userByIds = getUserByIds(userIds);
        System.out.println("userByIds start");
        System.out.println(userByIds);
        System.out.println("userByIds end");
        List<DgCommentResp> data = page.getRecords().stream().map(v -> {
            RbMember member = userByIds.get(v.getUserId().toString());
     ...
     * @return
     */
    public Map<String, RbMember> getUserByIds(List<Long> userIds){
        return userFeignClient.getMemberByIds(userIds).getData();
    }
    ...
    public List<DgCommentResp> ChildAll(DgCommentGetReq req){
        ...
        for (DgCommentResp item:respList){
            ...
            RbMember member = userFeignClient.getMember(memberReq);
            RbMember replyMember = userFeignClient.getMember(replyMemberReq);
            item.setAvatar(member.getAvatar());
            item.setName(member.getName());
